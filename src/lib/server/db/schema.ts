...
export const tableAuthZitadelUser = sqliteTable(
	'auth_zitadel_user',
	{
		id: int().primaryKey({ autoIncrement: true }),
		userId: int()
		...
		nik: text(),
		role: text(),
		isOnboarded: int({ mode: 'boolean' }).default(false).notNull(),
		rawMetadata: text({ mode: 'json' }).$type<{
			ptk_id?: string;
			dapodik_id?: string;
			source?: string;
 ...
 */
export const tableBobotNilaiAkhirMapel = sqliteTable(
	'bobot_nilai_akhir_mapel',
	{
		...
		mataPelajaranId: int()
			.references(() => tableMataPelajaran.id, { onDelete: 'cascade' })
			.notNull(),
		bobotFormatif: int().default(30).notNull(),
		bobotSumatif: int().default(70).notNull(),
		...audit
	},
 ...
 */
export const tableNilaiAkhirMapel = sqliteTable(
	'nilai_akhir_mapel',
	{
		id: int().primaryKey({ autoIncrement: true }),
		muridId: int()
